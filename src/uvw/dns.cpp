...
namespace uvw {
    ...
    UVW_INLINE_SPECIFIER void GetAddrInfoReq::addrInfoCallback(uv_getaddrinfo_t *req, int status, addrinfo *res) {
    ...
    UVW_INLINE_SPECIFIER void GetAddrInfoReq::nodeAddrInfo(const char *node, const char *service, addrinfo *hints) {
        invoke(&uv_getaddrinfo, parent(), get(), &addrInfoCallback, node, service, hints);
    }

    UVW_INLINE_SPECIFIER auto GetAddrInfoReq::nodeAddrInfoSync(const char *node, const char *service, addrinfo *hints) {
        auto req = get();
    ...
    UVW_INLINE_SPECIFIER void GetAddrInfoReq::nodeAddrInfo(std::string node, addrinfo *hints) {
        nodeAddrInfo(node.data(), nullptr, hints);
    }

    UVW_INLINE_SPECIFIER std::pair<bool, std::unique_ptr<addrinfo, GetAddrInfoReq::Deleter>>
    GetAddrInfoReq::nodeAddrInfoSync(std::string node, addrinfo *hints) {
        return nodeAddrInfoSync(node.data(), nullptr, hints);
    }

    UVW_INLINE_SPECIFIER void GetAddrInfoReq::serviceAddrInfo(std::string service, addrinfo *hints) {
    ...
    UVW_INLINE_SPECIFIER void GetAddrInfoReq::addrInfo(std::string node, std::string service, addrinfo *hints) {
    ...
    GetNameInfoReq::nameInfoCallback(uv_getnameinfo_t *req, int status, const char *hostname, const char *service) {
        auto ptr = reserve(req);
        if (status) {
            ptr->publish(ErrorEvent{status});
        }
    ...
    UVW_INLINE_SPECIFIER void GetNameInfoReq::nameInfo(const sockaddr &addr, int flags) {
        invoke(&uv_getnameinfo, parent(), get(), &nameInfoCallback, &addr, flags);
    }

    UVW_INLINE_SPECIFIER std::pair<bool, std::pair<const char *, const char *>>
    GetNameInfoReq::nameInfoSync(const sockaddr &addr, int flags) {
        auto req = get();
