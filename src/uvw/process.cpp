...
namespace uvw {
    ...
    void ProcessHandle::exitCallback(uv_process_t *handle, int64_t exitStatus, int termSignal) {
        ProcessHandle &process = *(static_cast<ProcessHandle*>(handle->data));
        process.publish(ExitEvent{exitStatus, termSignal});
    }

    void ProcessHandle::disableStdIOInheritance() noexcept {
        uv_disable_stdio_inheritance();
    }

    bool ProcessHandle::kill(int pid, int signum) noexcept {
        return (0 == uv_kill(pid, signum));
    }

    bool ProcessHandle::init() {
    ...
    void ProcessHandle::spawn(const char *file, char **args, char **env) {
    ...
    void ProcessHandle::kill(int signum) {
        invoke(&uv_process_kill, get(), signum);
    }

    int ProcessHandle::pid() noexcept {
        return get()->pid;
    }

    ProcessHandle &ProcessHandle::cwd(std::string path) noexcept {
        poCwd = path;
        return *this;
    }

    ProcessHandle &ProcessHandle::flags(Flags<Process> flags) noexcept {
        poFlags = flags;
        return *this;
    }

    ProcessHandle &ProcessHandle::stdio(FileHandle fd, Flags<StdIO> flags) {
    ...
    ProcessHandle &ProcessHandle::uid(Uid id) {
        poUid = id;
        return *this;
    }

    ProcessHandle &ProcessHandle::gid(Gid id) {
        poGid = id;
        return *this;
    }
}
